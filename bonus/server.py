...
"""
1 - requires the user to log in, save the token it returns and use it for all future requests.
2 - It also has a simple database so we can check that data we send to it is successfully returned
"""
...
def get_db():
    ...
    if db is None:
        ...
        with db:
            try:
                db.execute("CREATE TABLE numbers_table (name TEXT NOT NULL, number INTEGER NOT NULL UNIQUE)")
            except:
                pass
